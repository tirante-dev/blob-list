...
6. Do not include `data/chainlist/chains.json` or generated `artifacts/` changes in attribution PRs; CI posts the projected generated-data diff on the pull request and opens or updates a generated-data PR after merge.
...
If you add a new chain reference, you may run `npm run fetch-chainlist` locally before validation, but do not commit the resulting `data/chainlist/chains.json` change in attribution PRs. CI projects that snapshot for review and opens or updates a generated-data PR after merge.
