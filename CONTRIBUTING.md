...
6. Do not include `data/chainlist/snapshot.json` or generated `artifacts/` changes in attribution PRs; CI posts the projected generated-data diff on the pull request and publishes an `at/generated-data` branch for a generated-data PR after merge.
...
If you add a new chain reference, you may run `npm run fetch-chainlist` locally before validation, but do not commit the resulting `data/chainlist/snapshot.json` change in attribution PRs. CI projects that snapshot for review and publishes an `at/generated-data` branch for a generated-data PR after merge. Do not edit the snapshot by hand or add chain metadata to entity YAML; chain facts belong upstream in Chainlist.
