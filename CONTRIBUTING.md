...
6. Do not commit `data/chainlist/snapshot.json` or generated `artifacts/` files; they are gitignored. CI posts the projected generated-data diff on the pull request and publishes the dataset to the `generated-data` branch after merge.
...
Run `npm run fetch-chainlist` before validating; it writes the gitignored `data/chainlist/snapshot.json` lockfile that generation and validation read, including entries for any new chain references you add. CI projects that snapshot for review on the pull request and publishes it to the `generated-data` branch after merge. Do not edit the snapshot by hand or add chain metadata to entity YAML; chain facts belong upstream in Chainlist.
